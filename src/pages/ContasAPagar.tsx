...
import { ArrowLeft, ChevronRight, ChevronLeft, Sparkles, DollarSign, TrendingDown, CheckCircle, Clock, LogOut } from "lucide-react";
                ...
                {Array.from({ length: totalPaginas }, (_, i) => i + 1)
                  ...
                  .map((p, i) => p === "…"
                    ...
                    : <button key={p} onClick={() => setPagina(p as number)}
                        className={`flex h-7 min-w-[28px] items-center justify-center rounded-lg border px-2 text-xs font-medium transition-all ${paginaAtual === p ? "border-amber-400/30 bg-amber-400/10 text-amber-300" : "border-white/10 bg-white/5 text-slate-400 hover:border-white/20 hover:text-white"}`}>
                        {p}
                      </button>
                  )}
